...
// this hassle transaction demonstrates that Bob cannot hassle Alice
// by using her signature to spend to the escrow address after the expiration
// time without adding his own signature. this is the case because the CLTV
// opcode effectively mandates that the nLockTime be set and since Alice did
// not sign a transaction with nLockTime set, Bob can not use her signature
// to execute the CLTV script code path.
var hassleTransaction = new bitcore.Transaction().from({
  txid: earnestMoneyTransaction.id,
  vout: 0,
  scriptPubKey: earnestMoneyRedeemScript.toScriptHashOut(),
  satoshis: Number(args.satoshis) - 100000,
...
hassleTransaction.inputs[0].sequenceNumber = 0; // the CLTV opcode requires that the input's sequence number not be finalized

// setup the scriptSig of the hassle transaction to spend the p2sh-cltv redeem script
hassleTransaction.inputs[0].setScript(
  bitcore.Script.empty()
  .add(aliceSignature.toTxFormat())
  .add('OP_FALSE') // choose the withdraw offer code path
  .add(earnestMoneyRedeemScript.toBuffer())
);
...
var result = {
  ...
  hassleTransaction: {
    txid: hassleTransaction.id,
    raw: hassleTransaction.serialize(true),
  },
};
